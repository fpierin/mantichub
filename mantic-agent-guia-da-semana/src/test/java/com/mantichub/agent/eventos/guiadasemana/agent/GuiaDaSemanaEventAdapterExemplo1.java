...
public class GuiaDaSemanaEventAdapterExemplo1 {
	...
	public void init() throws Exception {
		...
		crawler = new GuiaDaSemanaEventAdapter(fromFile(arquivo));
	}
	...
	// final EventResource crawler = new
	// GuiaDaSemanaEventAdapter(fromFile(arquivo));
	...
	// + " <td width=\"130\" align=\"right\" class=\"desc-align\">Data<img
	// src=\"http://www.eventos.usp.br/wp-content/themes/eventos2011b/images/evento-data-ico.png\"
	// class=\"img-align\" alt=\"data\" /></td>\n"
	// + " <td>07/02/17 | 20:00 - 21:00<br /></td>\n" + " </tr>";
	// // // final String regex = ;
	// final EventResource crawler = new GuiaDaSemanaEventAdapter(fromFile);
	// GuiaDaSemanaEventAdapter.START_DATE_PATTERN =
	// "<td>(\\d{2}/\\d{2}/\\d{2})( - \\d{2}/\\d{2}/\\d{2})? \\| \\d{2}:\\d{2} -
	// \\d{2}:\\d{2}<br>";
	// final Date valorEsperado = new
	// SimpleDateFormat("dd/MM/yy").parse("07/02/17");
	// final Date valorEncontrado = crawler.getStartDate();
	// assertThat(valorEncontrado, is(valorEsperado));
	//
	// // final String valueByPattern = HTMLUtils.valueByPattern(fromFile,
	// "<td>(\\d{2}/\\d{2}/\\d{2})( -
	...
	// final EventCrawler crawler = new
	// EventoUSPEventCrawler(fromFile(arquivo));
	// final String valorEsperado = "A curadora Katia Canton, vice-diretora do
	// Museu de Arte Contemporânea (MAC) da USP,
	// se inspira no poema de Vinícius de Moraes para a exposição A Casa que
	// será inaugurada no dia 12 de setembro, a
	// partir das 11 horas. \n"
	// + "A Casa apresenta 18 obras que pertencem ao acervo do MAC, dos artistas
	// Alexander Calder, Leda Catunda, Alex
	// Flemming, Iran do Espírito Santo, Cildo Meireles, Nina Moraes, Alex
	// Vallauri, Flávio Cerqueira, Barrão, Regina
	// Silveira, Maria Tomaselli, Camille Kachani, Ângelo Venosa, José Carratu e
	// Ana Teixeira. Distribuídas não por
	// ordem cronológica ou autoral, mas pela representação dos papéis que cada
	// uma cumpriria em sua função de
	// domesticidade, as obras permitem pensar possibilidades, por exemplo, caso
	// o sofá de Regina Silveira fosse feito
	// para sentar, ou a vitrola de Iran do Espírito Santo fosse pensada para
	// tocar discos. Ou ainda as escumadeiras
	// espetadas no cordeiro, de Alex Flemming, fossem feitas para fritar,
	// utilizando para isso o fogão de Alex
	// Vallauri.\n"
	// + "A exposição integra a pesquisa Temas da Arte Contemporânea, em que
	// Katia discute como as novas propostas de
	// apreciação e aprendizado da arte podem ser realizadas sob uma variedade
	// de recortes, com temporalidades, temas e
	// enunciados diferentes e muitas vezes justapostos.\n"
	// + "O MAC USP Ibirapuera fica localizado na Avenida Pedro Álvares Cabral,
	// 1301 em São Paulo. A exposição será
	...
	// EventoUSPEventCrawler(fromFile(arquivo));
	// final Date valorEsperado = new
	// SimpleDateFormat("dd/MM/yy").parse("31/07/16");
