...
public class EventoUSPEventCrawlerExemplo1 {
	...
	private final static String arquivo = "src/test/resources/exemplo1.html";
	...
	public void verificaOverview() throws Exception {
		final EventCrawler crawler = new EventoUSPEventCrawler(fromFile(arquivo));
		final String valorEsperado = "A curadora Katia Canton, vice-diretora do Museu de Arte Contemporânea (MAC) da USP, se inspira no poema de Vinícius de Moraes para a exposição A Casa que será inaugurada no dia 12 de setembro, a partir das 11 horas. \n"
	...
	public void verificaDataInicio() throws Exception {
		final EventCrawler crawler = new EventoUSPEventCrawler(fromFile(arquivo));
		final Date valorEsperado = new SimpleDateFormat("dd/MM/yy").parse("12/09/15");
	...
	public void verificaEndereco() throws Exception {
		final EventCrawler crawler = new EventoUSPEventCrawler(fromFile(arquivo));
		final String valorEsperado = "Avenida Pedro Álvares Cabral, 1301";
